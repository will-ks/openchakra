...
import { ComponentType, IComponent, IComponents, Ocho } from '~core/Ocho'
...
// components must be dynamically initialized as it contains some configurable paraeters, eg. templates
let components: ReturnType<typeof initComponents> = {} as any
export function initComponents(ocho: Ocho) {
  const model = createModel({
    ...
    reducers: {
      ...
      loadDemo(state: ComponentsState, type: string): ComponentsState {
        return {
          ...state,
          selectedId: 'comp-root',
          components: ocho.templates ? ocho.templates[type] : {},
        }
      },
      resetProps(state: ComponentsState, componentId: string): ComponentsState {
        return produce(state, (draftState: ComponentsState) => {
          ...
          const { form, ...defaultProps } =
            ocho.previewDefaultProps[component.type] || {}
      ...
      addComponent(
        ...
        return produce(state, (draftState: ComponentsState) => {
          ...
          const { form, ...defaultProps } =
            ocho.previewDefaultProps[payload.type] || {}
